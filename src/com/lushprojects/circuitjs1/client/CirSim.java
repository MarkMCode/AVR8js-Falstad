//Edited by Mark Megarry in June-July 2020 for use in ThreeFalstad, original code by Paul Falstad and Iain Sharp
/*    
...
MouseOutHandler, MouseWheelHandler {
    ...
    Random random;
    Button resetButton;
    //Button infoButton;	//ADDED BY MARK MEGARRY
...
//		String doc = applet.getDocumentBase().toString();
	...
	loadShortcuts();
	  ...
	  RootPanel.get("FalstadDiv").add(RootLayoutPanel.get());	//Added by Mark Megarry
	...
	cv =Canvas.createIfSupported();
	 ...
	 backcv=Canvas.createIfSupported();
		...
		verticalPanel.add(buttonPanel);
		 ...
		 //INFO BUTTON ADDED BY MARK MEGARRY
		 /*verticalPanel.add(infoButton = new Button(LS("Info")));
		 infoButton.addClickHandler(new ClickHandler() {
			    public void onClick(ClickEvent event) {
			      infoAction();
			    }
			  });*/
		 /*infoButton.setStylePrimaryName("topButton");
		 //ELEMENT INFO BUTTON ADDED BY MARK MEGARRY
		 verticalPanel.add(elmButton = new Button(LS("Show/hide element numbers")));
		 elmButton.addClickHandler(new ClickHandler() {
			    public void onClick(ClickEvent event) {
			      elmAction();
			    }
			  });
...
//	g.clipRect(0, 0, circuitArea.width, circuitArea.height);
	...
	//Check elmInfo flag and draw elm info if true
	if(elmInfoFlag) {	//ADDED BY MARK MEGARRY
	    ...
	    Context2d context = cv.getContext2d();	//Create context2d from cv
	    	for(int elm = 0; elm<elmList.size(); elm++) {
	    	    	...
	    	    	//if(currentElm.getInfo(0) == "voltage source") {
...
//	System.out.println((System.currentTimeMillis()-lastFrameTime)/(double) iter);
    ...
    //infoAction ADDED BY MARK MEGARRY
   /* public void infoAction() {	//Display value of selected node and inc. selected node
	...
	infoButton.setHTML(buttonText);	//Write buttonText to button
    ...
    //elmAction ADDED BY MARK MEGARRY
    public void elmAction() {
	elmInfoFlag = !elmInfoFlag;
	updateCircuit();
    }*/
